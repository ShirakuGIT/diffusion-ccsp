...
class GlobalEnergyMLP(nn.Module):
...
@dataclass
class LearnedGlobalEnergy:
    model: GlobalEnergyMLP
    mean: np.ndarray
    std: np.ndarray
    max_nodes: int


def train_global_energy_model(
    x_np: np.ndarray,
    y_np: np.ndarray,
    epochs: int = 10,
    batch_size: int = 128,
    ...
    device: str = 'cpu',
    max_nodes: int = 0,
) -> tuple[LearnedGlobalEnergy, Dict[str, object]]:
    ...
    if np.isnan(x_norm).any():
        raise ValueError('NaNs detected after global feature normalization.')
    ...
    model = GlobalEnergyMLP(input_dim=x_np.shape[1]).to(device)
