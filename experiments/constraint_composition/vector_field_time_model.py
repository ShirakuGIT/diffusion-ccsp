...
def train_time_vector_field_model(
    x_np: np.ndarray,
    v_np: np.ndarray,
    epochs: int = 10,
    batch_size: int = 128,
    ...
    device: str = 'cpu',
    max_nodes: int = 0,
) -> tuple[LearnedTimeVectorField, Dict[str, object]]:
    ...
    if np.isnan(x_norm).any():
        raise ValueError('NaNs detected after time-conditioned feature normalization.')
    ...
    model = TimeConditionedMLP(input_dim=x_np.shape[1], output_dim=v_np.shape[1]).to(device)
